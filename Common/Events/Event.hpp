...
public:
	...
	virtual inline string GetClassName() const { return "Event"; }

	inline EventID GetID() const { return m_ID; }

	inline IEventDispatcher* GetTarget() const { return mp_EventTarget; }
	template <typename T>
	inline T* GetTargetAs() const { return dynamic_cast<T*>(mp_EventTarget); }

	inline void setTarget(IEventDispatcher* pTarget) { mp_EventTarget = pTarget; }

	inline EventData* GetData() const { return mp_Data; }
	template <typename T>
	inline T* GetDataAs() const { return dynamic_cast<T*>(mp_Data); }
